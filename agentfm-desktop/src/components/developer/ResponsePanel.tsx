...
export function ResponsePanel({ result, loading }: Props) {
  if (loading) {
    return <div className="bg-bg-2 border border-border-0 rounded-2xl p-4 text-sm text-text-2">Sending…</div>
  }
  if (!result) {
    return <div className="bg-bg-2 border border-border-0 rounded-2xl p-4 text-sm text-text-2">Send a request to see the response.</div>
  }
  if (result.status === 0) {
    return (
      <div className="bg-bg-2 border border-border-0 rounded-2xl p-4 text-sm">
  ...
  const ok = result.ok
  return (
    <div className="bg-bg-2 border border-border-0 rounded-2xl p-4">
